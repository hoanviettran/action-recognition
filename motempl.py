...
# def compute_mhi_image(frame, prev_frame, MHI_DURATION):
...
if __name__ == '__main__':
    ...
    videopath = '/home/hoantranviet/Downloads/rawvideo/2.mp4'
    ...
    while True:
        ...
        seg_mask, seg_bounds = cv.motempl.segmentMotion(motion_history, timestamp, 0.5)
        ...
        # path = '/media/hoantranviet/data/ImageProcessing/Frame-crop/'
        for i, rect in enumerate([(0, 0, w, h)] + list(seg_bounds)):
            ...
            draw_motion_comp(vis, rect[0], rect[1], rect[2], rect[3], angle, color)

            # cv.imwrite(path + str(i) + '_' + str(cam.get(1)) + '.jpg', vis[y:y+rh,x:x+rw])
        cv.putText(vis, visual_name, (20, 20), cv.FONT_HERSHEY_PLAIN, 1.0, (200,0,0))
        cv.imshow('motempl', vis)


        prev_frame = frame.copy()
        if 0xFF & cv.waitKey(5) == ord('q'):
            break
